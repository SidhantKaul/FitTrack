...
function App() {
  ...
  const [disarr,setDisArr] = useState(true);
  function passAdd() {
    setDisplayAdd(false);
  ...
  function Date(key) {
    ...
    setDate(prevValue => {
      if(prevValue!==key) {
        ...
        axios
        .get("/getExercises/"+key,{withCredentials: true})
        .then(res => {
  ...
  function afterDel() {
    console.log(">?>?>?>?>?>?");
    axios
    .get("/getExercises/"+date,{withCredentials: true})
    .then(res => {
      console.log(res.data);
      if(res.data) {
        setExerciseArray(res.data);
  ...
  return (
      ...
      <Router>
        <Switch>
          <Route exact path="/">
        <Navbar Date={Date} img={img} log={afterlog}/>
